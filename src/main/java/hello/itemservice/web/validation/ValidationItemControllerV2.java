...
public class ValidationItemControllerV2 {
    ...
    /**
     * 얘는 언제 호출되냐면 이 컨트롤러가 호출 될때마다 실행된다.
     * 각 요청이 들어올때마다 모든 요청의 메서드에 실행된다고 보면됨.
      */
    @InitBinder
...
//    @PostMapping("/add")
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if(item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(new FieldError("item", "price", item.getPrice(), false, null, null, "가격은 1,000 ~ 1,000,000까지 혀용합니다."));
        }
        if(item.getQuantity() == null || item.getQuantity() >= 9999) {
            bindingResult.addError(new FieldError("item", "quantity", item.getQuantity(), false, null, null, "수량은 최대 9,999까지 혀용합니다."));
        }
...
//    @PostMapping("/add")
    public String addItemV5(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        itemValidator.validate(item, bindingResult); // 이 한줄로 검증 로직을 호출함.
    ...
    // @Validated 하나 넣어주면 Item에 대해서 자동으로 검증기가 수행이 됨.
    @PostMapping("/add")
